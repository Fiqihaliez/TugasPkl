...
<div class="container mx-auto px-4 py-8 flex justify-center">
    <div class="w-full max-w-md bg-white p-6 rounded-lg shadow-lg">
        ...
        <form id="edit-course-form" enctype="multipart/form-data">
            ...
            <div class="mb-4 text-center">
                ...
                <div id="image-preview">
                    @if ($course->image_url)
                        <img id="current-image" src="{{ asset('storage/' . $course->image_url) }}" alt="Course Image" class="mt-4 mx-auto max-h-32 object-cover">
                    @endif
                </div>
            </div>
        ...
        <a href="{{ route('courses.index') }}" class="block text-center mt-4 text-blue-500">Back to Courses</a>
    </div>
</div>
@endsection
    ...
    $('#edit-course-form').on('submit', function (e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                $('#current-image').attr('src', response.new_image_url + '?timestamp=' + new Date().getTime());
                Swal.fire({
                    title: 'Updated!',
